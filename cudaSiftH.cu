...
void ExtractSiftOctave(SiftData &siftData, CudaImage &img, int octave, float thresh,
  ...
  int h = img.height;
  int p = iAlignUp(w, 128);
  for (int i = 0; i < nd - 1; i++) diffImg[i].Allocate(w, h, p, false, memoryTmp + i * p * h);
...
void PrintSiftData(SiftData &data) {
  ...
  if (data.h_data == NULL) {
    ...
    safeCall(
        cudaMemcpy(h_data, data.d_data, sizeof(SiftPoint) * data.numPts, cudaMemcpyDeviceToHost));
    data.h_data = h_data;
  }
#endif
...
double ScaleDown(CudaImage &res, CudaImage &src, float variance) {
  ...
  ScaleDown<<<blocks, threads>>>(res.d_data, src.d_data, src.width, src.pitch, src.height,
                                 res.pitch);
#endif
...
#else
  dim3 threads(256);
  ComputeOrientationsCONSTNew<<<blocks, threads>>>(src.d_data, src.width, src.pitch, src.height,
...
double ExtractSiftDescriptors(cudaTextureObject_t texObj, SiftData &siftData, float subsampling,
                              int octave) {
  dim3 blocks(512);
  dim3 threads(16, 8);
#ifdef MANAGEDMEM
...
double OrientAndExtract(cudaTextureObject_t texObj, SiftData &siftData, float subsampling,
                        int octave) {
  dim3 blocks(256);
  dim3 threads(128);
#ifdef MANAGEDMEM
...
double LowPass(CudaImage &res, CudaImage &src, float scale) {
  ...
  if (scale != oldScale) {
    ...
    for (int j = -LOWPASS_R; j <= LOWPASS_R; j++) kernel[j + LOWPASS_R] /= kernelSum;
...
void PrepareLaplaceKernels(int numOctaves, float initBlur, float *kernel) {
  ...
  for (int i = 0; i < NUM_SCALES + 3; i++) {
    ...
    for (int j = 0; j <= LAPLACE_R; j++) kernel[numOctaves * 12 * 16 + 16 * i + j] /= kernelSum;
    scale *= diffScale;
  }
}

double LaplaceMulti(cudaTextureObject_t texObj, CudaImage &baseImage, CudaImage *results,
                    int octave) {
...
#ifdef MANAGEDMEM
  FindPointsMulti<<<blocks, threads>>>(sources->d_data, siftData.m_data, w, p, h, subsampling,
