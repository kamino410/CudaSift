...
///////////////////////////////////////////////////////////////////////////////
__global__ void ScaleDownDenseShift(float *d_Result, float *d_Data, int width, int pitch,
...
#define H2 (SCALEDOWN_H / 2)
  ...
  if (xp < (width + 4) && yp < (height + 4)) {
    ...
    brows[BW * ty + tx] = k0 * (v + ShiftDown(v, 4)) + k1 * (ShiftDown(v, 1) + ShiftDown(v, 3)) +
                          k2 * ShiftDown(v, 2);
  }
  __syncthreads();
...
__global__ void ScaleDownDense(float *d_Result, float *d_Data, int width, int pitch, int height,
                               int newpitch) {
#define BW (SCALEDOWN_W + 4)
#define BH (SCALEDOWN_H + 4)
#define W2 (SCALEDOWN_W / 2)
#define H2 (SCALEDOWN_H / 2)
  ...
  const float k1 = d_ScaleDownKernel[1];
  const float k2 = d_ScaleDownKernel[2];
  if (xp < (width + 4) && yp < (height + 4)) irows[BW * ty + tx] = d_Data[yl * pitch + xl];
  __syncthreads();
...
__global__ void ScaleDown(float *d_Result, float *d_Data, int width, int pitch, int height,
                          int newpitch) {
...
__global__ void ScaleUp(float *d_Result, float *d_Data, int width, int pitch, int height,
                        int newpitch) {
  const int tx = threadIdx.x;
  const int ty = threadIdx.y;
...
__global__ void ExtractSiftDescriptors(cudaTextureObject_t texObj, SiftPoint *d_sift, int fstPts,
                                       float subsampling) {
...
__global__ void ExtractSiftDescriptorsCONSTNew(cudaTextureObject_t texObj, SiftPoint *d_sift,
  ...
  // if (tx==0 && ty==0)
  //  printf("%d %d %d %d\n", octave, fstPts, min(d_PointCounter[2*octave], d_MaxNumPoints),
  //  totPts);
...
__global__ void ExtractSiftDescriptorsCONST(cudaTextureObject_t texObj, SiftPoint *d_sift,
                                            float subsampling, int octave) {
  __shared__ float gauss[16];
  __shared__ float buffer[128];
  __shared__ float sums[4];
...
__global__ void ExtractSiftDescriptorsOld(cudaTextureObject_t texObj, SiftPoint *d_sift, int fstPts,
                                          float subsampling) {
  __shared__ float gauss[16];
  __shared__ float buffer[128];
  ...
  // Normalize twice and suppress peaks first time
  if (idx < 64) sums[idx] = buffer[idx] * buffer[idx] + buffer[idx + 64] * buffer[idx + 64];
  __syncthreads();
...
__device__ void ExtractSiftDescriptor(cudaTextureObject_t texObj, SiftPoint *d_sift,
                                      float subsampling, int octave, int bx) {
  __shared__ float gauss[16];
  __shared__ float buffer[128];
  __shared__ float sums[4];
...
__global__ void ComputeOrientationsCONSTNew(float *image, int w, int p, int h, SiftPoint *d_Sift,
                                            int octave) {
#define RAD 9
#define WID (2 * RAD + 1)
#define LEN 32  //%%%% Note: Lowe suggests 36, not 32
  ...
  for (int bx = blockIdx.x + fstPts; bx < totPts; bx += gridDim.x) {
    ...
    fac[2] = 1.0f;
    float i2sigma2 =
        -1.0f / (2.0f * 2.0f * 2.0f * sc * sc);  //%%%% Note: Lowe suggests 1.5, not 2.0
    if (tx < WID) {
    ...
    if (tx < LEN) {
      ...
      hist[tx + LEN] =
          8.0f * hist[tx] + 4.0f * (hist[x1m] + hist[x1p]) + 0.0f * (hist[x2m] + hist[x2p]);
...
// With constant number of blocks
__global__ void ComputeOrientationsCONST(cudaTextureObject_t texObj, SiftPoint *d_Sift,
                                         int octave) {
  __shared__ float hist[64];
  __shared__ float gauss[11];
  const int tx = threadIdx.x;
...
// With constant number of blocks
__global__ void OrientAndExtractCONST(cudaTextureObject_t texObj, SiftPoint *d_Sift,
                                      float subsampling, int octave) {
  __shared__ float hist[64];
  __shared__ float gauss[11];
...
__global__ void FindPointsMultiTest(float *d_Data0, SiftPoint *d_Sift, int width, int pitch,
                                    int height, float subsampling, float lowestScale, float thresh,
...
#define MEMWID (MINMAX_W + 2)
  ...
  if (ty == 0 && tx < cnt) {
    ...
    float dxy =
        0.25f * (data1[+pitch + 1] + data1[-pitch - 1] - data1[-pitch + 1] - data1[+pitch - 1]);
    float tra = dxx + dyy;
...
__global__ void FindPointsMultiNew(float *d_Data0, SiftPoint *d_Sift, int width, int pitch,
                                   int height, float subsampling, float lowestScale, float thresh,
                                   float factor, float edgeLimit, int octave) {
#define MEMWID (MINMAX_W + 2)
...
__global__ void FindPointsMulti(float *d_Data0, SiftPoint *d_Sift, int width, int pitch, int height,
                                float subsampling, float lowestScale, float thresh, float factor,
...
__global__ void FindPointsMultiOld(float *d_Data0, SiftPoint *d_Sift, int width, int pitch,
                                   int height, float subsampling, float lowestScale, float thresh,
                                   float factor, float edgeLimit, int octave) {
#define MEMWID (MINMAX_W + 2)
...
__global__ void LaplaceMultiTex(cudaTextureObject_t texObj, float *d_Result, int width, int pitch,
  ...
  if (tx < LAPLACE_W) {
    sdata2[tx] = kernel[0] * sdata1[tx + 4] + kernel[1] * (sdata1[tx + 3] + sdata1[tx + 5]) +
...
__global__ void LaplaceMultiMemWide(float *d_Image, float *d_Result, int width, int pitch,
                                    int height, int octave) {
  __shared__ float buff[(LAPLACE_W + 2 * LAPLACE_R) * LAPLACE_S];
  const int tx = threadIdx.x;
  const int xp = blockIdx.x * LAPLACE_W + tx;
  ...
  float temp[9];
  if (xp < (width + 2 * LAPLACE_R)) {
    for (int i = 0; i < 4; i++) temp[i] = data[max(0, min(yp + i - 4, height - 1)) * pitch];
    for (int i = 4; i < 8 + 1; i++) temp[i] = data[min(yp + i - 4, height - 1) * pitch];
    for (int scale = 0; scale < LAPLACE_S; scale++) {
      float *kernel = d_LaplaceKernel + octave * 12 * 16 + scale * 16;
      for (int i = 0; i <= LAPLACE_R; i++) kern[scale][i] = kernel[LAPLACE_R - i];
      float *buf = buff + (LAPLACE_W + 2 * LAPLACE_R) * scale;
      buf[tx] = kern[scale][4] * temp[4] + kern[scale][3] * (temp[3] + temp[5]) +
                kern[scale][2] * (temp[2] + temp[6]) + kern[scale][1] * (temp[1] + temp[7]) +
  ...
  if (tx < LAPLACE_W / 4 && xp4 < width) {
    ...
    for (int scale = 1; scale < LAPLACE_S; scale++) {
      ...
      reinterpret_cast<float4 *>(&d_Result[(scale - 1) * height * pitch + yp * pitch + xp4])[0] =
          dif4;
      old4 = new4;
    }
  }
...
__global__ void LaplaceMultiMemTest(float *d_Image, float *d_Result, int width, int pitch,
                                    int height, int octave) {
  __shared__ float data1[(LAPLACE_W + 2 * LAPLACE_R) * LAPLACE_S];
  __shared__ float data2[LAPLACE_W * LAPLACE_S];
  const int tx = threadIdx.x;
  ...
  for (int i = 0; i < 4; i++) temp[i] = data[max(0, min(yp + i - 4, h)) * pitch];
  for (int i = 4; i < 8 + LAPLACE_H; i++) temp[i] = data[min(yp + i - 4, h) * pitch];
  for (int i = 0; i <= LAPLACE_R; i++) kern[i] = kernel[LAPLACE_R - i];
  for (int j = 0; j < LAPLACE_H; j++) {
    sdata1[tx] = kern[4] * temp[4 + j] + kern[3] * (temp[3 + j] + temp[5 + j]) +
                 kern[2] * (temp[2 + j] + temp[6 + j]) + kern[1] * (temp[1 + j] + temp[7 + j]) +
                 kern[0] * (temp[0 + j] + temp[8 + j]);
    __syncthreads();
    float *sdata2 = data2 + LAPLACE_W * scale;
    if (tx < LAPLACE_W) {
      sdata2[tx] = kern[4] * sdata1[tx + 4] + kern[3] * (sdata1[tx + 3] + sdata1[tx + 5]) +
...
__global__ void LaplaceMultiMemOld(float *d_Image, float *d_Result, int width, int pitch,
                                   int height, int octave) {
  __shared__ float data1[(LAPLACE_W + 2 * LAPLACE_R) * LAPLACE_S];
  __shared__ float data2[LAPLACE_W * LAPLACE_S];
  const int tx = threadIdx.x;
...
__global__ void LowPass(float *d_Image, float *d_Result, int width, int pitch, int height) {
  ...
  if (tx < LOWPASS_W && xp < width && yp < height)
    d_Result[yp * pitch + xp] =
        kernel[4] * buff[tx + 4] + kernel[3] * (buff[tx + 3] + buff[tx + 5]) +
        kernel[2] * (buff[tx + 2] + buff[tx + 6]) + kernel[1] * (buff[tx + 1] + buff[tx + 7]) +
...
__global__ void LowPassBlockOld(float *d_Image, float *d_Result, int width, int pitch, int height) {
  ...
  for (int l = -8; l <= LOWPASS_H; l += 4) {
    if (l < LOWPASS_H) {
      ...
      xrows[(l + 8 + ty) % N][tx] =
          k[4] * ShiftDown(val, 4) + k[3] * (ShiftDown(val, 5) + ShiftDown(val, 3)) +
          k[2] * (ShiftDown(val, 6) + ShiftDown(val, 2)) +
          k[1] * (ShiftDown(val, 7) + ShiftDown(val, 1)) + k[0] * (ShiftDown(val, 8) + val);
    }
    if (l >= 4) {
      int ys = yp + l - 4;
...
#pragma unroll
  for (int l = -8; l < 4; l += 4) {
    ...
    int yl = max(min(yp + l + 4, height - 1), 0);
    float val = d_Image[yl * pitch + xl];
    val = k[4] * ShiftDown(val, 4) + k[3] * (ShiftDown(val, 5) + ShiftDown(val, 3)) +
          k[2] * (ShiftDown(val, 6) + ShiftDown(val, 2)) +
          k[1] * (ShiftDown(val, 7) + ShiftDown(val, 1)) + k[0] * (ShiftDown(val, 8) + val);
    xrows[ly + 8][tx] = val;
  }
