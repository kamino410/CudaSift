...
__global__ void MatchSiftPoints(SiftPoint *sift1, SiftPoint *sift2, float *corrData, int numPts1,
                                int numPts2) {
  ...
  float sum = 0.0f;
  if (p2 < numPts2)
    for (int j = 0; j < 8; j++) sum += siftPoint[16 * j + tx] * ptr2[16 * j + tx];
  sums[i] = sum;
  __syncthreads();
...
__global__ void MatchSiftPoints2(SiftPoint *sift1, SiftPoint *sift2, float *corrData, int numPts1,
                                 int numPts2) {
  ...
  if (p1 < numPts1) corrData[p1 * gridDim.y * 16 + p2] = (p2 < numPts2 ? sum : -1.0f);
}

__global__ void FindMaxCorr(float *corrData, SiftPoint *sift1, SiftPoint *sift2, int numPts1,
                            int corrWidth, int siftSize) {
...
__global__ void FindMaxCorr3(float *corrData, SiftPoint *sift1, SiftPoint *sift2, int numPts1,
                             int numPts2) {
...
__global__ void FindMaxCorr4(SiftPoint *sift1, SiftPoint *sift2, int numPts1, int numPts2) {
  ...
  for (int p2 = 0; p2 < numPts2; p2++) {
    const float *pt2 = sift2[p2].data;
    float sum = 0.0f;
    for (int j = tx; j < 128; j += FMC2W) sum += siftPoint[128 * ty + j] * pt2[j];
    for (int j = FMC2W / 2; j > 0; j /= 2) sum += ShiftDown(sum, j);
    if (tx == 0) {
      if (sum > maxScore[ty]) {
        maxScor2[ty] = maxScore[ty];
  ...
  for (int bp2 = 0; bp2 < numPts2 - M7H + 1; bp2 += M7H) {
    ...
    if (idx < M7W * M7H / M7R / NRX) {
      ...
      for (int d = 0; d < NDIM / 4; d++) {
        ...
        for (int i = 0; i < NRX; i++)
          v1[i] =
              buffer1[((M7W / NRX) * i + ix) * NDIM / 4 + (d + (M7W / NRX) * i + ix) % (NDIM / 4)];
        for (int dy = 0; dy < M7R; dy++) {
...
__global__ void FindMaxCorr9(SiftPoint *sift1, SiftPoint *sift2, int numPts1, int numPts2) {
  ...
  for (int k = 0; k < min(FMC_GH, numPts2 - FMC_BH + 1); k += FMC_BH) {
    ...
    if (idx < FMC_BW)
      for (int i = 0; i < FMC_BD / 2; i++) siftParts1[(i + 0) * FMC_BW + idx] = pts1[0 + i];
    if (idx < FMC_BH)
      for (int i = 0; i < FMC_BD / 2; i++) siftParts2[(i + 0) * FMC_BH + idx] = pts2[0 + i];
    __syncthreads();

    int b = FMC_BD / 2;
    for (int d = FMC_BD / 2; d < 32; d += FMC_BD / 2) {
      if (idx < FMC_BW)
        for (int i = 0; i < FMC_BD / 2; i++) siftParts1[(i + b) * FMC_BW + idx] = pts1[d + i];
      if (idx < FMC_BH)
        for (int i = 0; i < FMC_BD / 2; i++) siftParts2[(i + b) * FMC_BH + idx] = pts2[d + i];
...
__global__ void FindMaxCorr8(SiftPoint *sift1, SiftPoint *sift2, int numPts1, int numPts2) {
  ...
  for (int k = 0; k < min(FMC_GH, numPts2 - FMC_BH + 1); k += FMC_BH) {
    ...
    for (int d = 0; d < 32; d += FMC_BD) {
      if (idx < FMC_BW)
        for (int i = 0; i < FMC_BD; i++) siftParts1[i * FMC_BW + idx] = pts1[d + i];
      if (idx < FMC_BH)
        for (int i = 0; i < FMC_BD; i++) siftParts2[i * FMC_BH + idx] = pts2[d + i];
      __syncthreads();

      for (int i = 0; i < FMC_BD; i++) {
        float4 v1[FMC_TW];
        for (int ix = 0; ix < FMC_TW; ix++) v1[ix] = siftParts1[i * FMC_BW + (tx * FMC_TW + ix)];
        for (int iy = 0; iy < FMC_TH; iy++) {
...
#pragma unroll
    ...
    if (ty < (16 / NUM))
      for (int l = 0; l < NUM; l++) sums[16 * (ty + l * (16 / NUM)) + tx] = sum[l];
    __syncthreads();
    if (ty == 0) {
      for (int j = 0; j < 16; j++) {
...
__global__ void FindMaxCorr5(SiftPoint *sift1, SiftPoint *sift2, int numPts1, int numPts2) {
  ...
  for (int k = 0; k < 512 / 16; k++) {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      __syncthreads();
      for (int j = 0; j < 16; j++) sum += siftParts1[17 * j + tx] * siftParts2[17 * j + ty];
      __syncthreads();
    }
    float *sums = siftParts1;
    sums[16 * ty + tx] = sum;
...
__device__ void InvertMatrix(float elem[size][size], float res[size][size]) {
  ...
  for (int j = 0; j < size; j++) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      b[ip] = b[i];
      if (ii != -1)
        for (int j = ii; j < i; j++) sum -= elem[i][j] * b[j];  // i>j (upper right)
      else if (sum != 0.0)
        ii = i;
      b[i] = sum;
    }
    for (int i = size - 1; i >= 0; i--) {
      float sum = b[i];
      for (int j = i + 1; j < size; j++) sum -= elem[i][j] * b[j];  // i<j (lower left)
...
__global__ void TestHomographies(float *d_coord, float *d_homo, int *d_counts, int numPts,
                                 float thresh2) {
  ...
  if (ty < 8 && tx < TESTHOMO_LOOPS) homo[tx * 8 + ty] = d_homo[idx + ty * numLoops];
  __syncthreads();
  float a[8];
  ...
  if (tx < TESTHOMO_LOOPS && ty == 0) d_counts[idx] = cnts[TESTHOMO_TESTS * tx];
  __syncthreads();
}
...
double FindHomography(SiftData &data, float *homography, int *numMatches, int numLoops,
  ...
  *numMatches = 0;
  ...
  safeCall(cudaMemcpy2D(h_ambiguities, szFl, &d_sift[0].ambiguity, szPt, szFl, numPts,
                        cudaMemcpyDeviceToHost));
  int *validPts = (int *)malloc(sizeof(int) * numPts);
  int numValid = 0;
  for (int i = 0; i < numPts; i++) {
    if (h_scores[i] > minScore && h_ambiguities[i] < maxAmbiguity) validPts[numValid++] = i;
  }
  free(h_scores);
  free(h_ambiguities);
  if (numValid >= 8) {
...
// Combined version with no global memory requirement using global locks
  ...
  if (data1.h_data != NULL) {
    ...
    safeCall(cudaMemcpy2D(h_ptr, sizeof(SiftPoint), d_ptr, sizeof(SiftPoint), 5 * sizeof(float),
