...
void MatchC1(float *h_pts1, float *h_pts2, float *h_score, int *h_index) {
  ...
  for (int p1 = 0; p1 < NPTS; p1++) {
    for (int p2 = 0; p2 < NPTS; p2++) {
      float score = 0.0f;
      for (int d = 0; d < NDIM; d++) score += h_pts1[p1 * NDIM + d] * h_pts2[p2 * NDIM + d];
      if (score > h_score[p1]) {
        h_score[p1] = score;
        h_index[p1] = p2;
      }
...
void CheckMatches(int *h_index, int *h_index2, float *h_score, float *h_score2) {
  ...
  for (int i = 0; i < NPTS; i++) {
    ...
    if (h_index[i] != h_index2[i])
      std::cout << "  " << i << " " << h_index[i] << " " << h_index2[i] << " " << h_score[i] << " "
                << h_score2[i] << std::endl;
  }
...
__global__ void Match1(float *d_pts1, float *d_pts2, float *d_score, int *d_index) {
  ...
  for (int p2 = 0; p2 < NPTS; p2++) {
    float score = 0.0f;
    for (int d = 0; d < NDIM; d++) score += d_pts1[p1 * NDIM + d] * d_pts2[p2 * NDIM + d];
    if (score > max_score) {
      max_score = score;
      index = p2;
    }
...
__global__ void Match2(float *d_pts1, float *d_pts2, float *d_score, int *d_index) {
  ...
  for (int bp2 = 0; bp2 < NPTS; bp2 += M2H) {
    ...
    float score = 0.0f;
    for (int d = 0; d < NDIM; d++) score += buffer1[tx * NDIM + d] * buffer2[ty * NDIM + d];  //%%%%
    scores[idx] = score;
    __syncthreads();
...
__global__ void Match3(float *d_pts1, float *d_pts2, float *d_score, int *d_index) {
  ...
  int index = -1;
  for (int bp2 = 0; bp2 < NPTS; bp2 += M2H) {
    for (int d = tx; d < NDIM; d += M2W) buffer2[ty * NDIM + d] = d_pts2[(bp2 + ty) * NDIM + d];
    __syncthreads();
...
__global__ void Match5(float *d_pts1, float *d_pts2, float *d_score, int *d_index) {
  ...
  for (int bp2 = 0; bp2 < NPTS; bp2 += M5H) {
    ...
    if (ty < M5H / M5R) {  //%%%%
      ...
      for (int dy = 0; dy < M5R; dy++) scores[tx + M5W * (M5R * ty + dy)] = score[dy];
    }
    __syncthreads();
...
#define NUM (NRX * M7R)                       // 32*8 threads
  ...
  for (int bp2 = 0; bp2 < NPTS; bp2 += M7H) {
    ...
    for (int dp = 0; dp < NDIM / 4; dp += NUM) {
      float4 temp;
      if (dp < (NDIM / 4 - NUM)) temp = ((float4 *)d_pts2)[(bp2 + j) * (NDIM / 4) + dp + d + NUM];
...
int main(int argc, char *argv[]) {
  ...
  std::cout << "Upload:      " << delay << " ms  " << 2 * psize * NDIM / delay / 1024 / 1024
            << " MB/ms" << std::endl;
  ...
  std::cout << "MatchGPU3:   " << delay << " ms  " << 2.0 * NPTS * NPTS * NDIM / delay / 1024 / 1024
            << " Gflops" << std::endl;
  ...
  std::cout << "MatchGPU4:   " << delay << " ms  " << 2.0 * NPTS * NPTS * NDIM / delay / 1024 / 1024
            << " Gflops" << std::endl;
  ...
  std::cout << "MatchGPU5:   " << delay << " ms  " << 2.0 * NPTS * NPTS * NDIM / delay / 1024 / 1024
            << " Gflops" << std::endl;
  ...
  std::cout << "MatchGPU6:   " << delay << " ms  " << 2.0 * NPTS * NPTS * NDIM / delay / 1024 / 1024
            << " Gflops" << std::endl;
  ...
  std::cout << "MatchGPU7:   " << delay << " ms  " << 2.0 * NPTS * NPTS * NDIM / delay / 1024 / 1024
            << " Gflops" << std::endl;
  ...
  std::cout << "MatchGPU8:   " << delay << " ms  " << 2.0 * NPTS * NPTS * NDIM / delay / 1024 / 1024
            << " Gflops" << std::endl;
#if 1
  blocks = dim3(NPTS / M7W);
  ...
  delay = time.read() - ltime;
  std::cout << "Download:    " << delay << " ms  " << 2 * psize / delay / 1024 / 1024 << " MB/ms"
            << std::endl;
  ltime = time.read();

  if (CHECK) CheckMatches(h_index.data(), h_index2.data(), h_score.data(), h_score2.data());
