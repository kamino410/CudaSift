...
int main(int argc, char **argv) {
  ...
  std::cout << "Number of original features: " << siftData1.numPts << " " << siftData2.numPts
            << std::endl;
  std::cout << "Number of matching features: " << numFit << " " << numMatches << " "
            << 100.0f * numFit / std::min(siftData1.numPts, siftData2.numPts) << "% " << initBlur
            << " " << thresh << std::endl;
  //}
...
void MatchAll(SiftData &siftData1, SiftData &siftData2, float *homography) {
  ...
  for (int i = 0; i < numPts1; i++) {
    ...
    for (int j = 0; j < numPts2; j++) {
      ...
      float dx =
          (homography[0] * sift1[i].xpos + homography[1] * sift1[i].ypos + homography[2]) / den -
          sift2[j].xpos;
      float dy =
          (homography[3] * sift1[i].xpos + homography[4] * sift1[i].ypos + homography[5]) / den -
          sift2[j].ypos;
...
void PrintMatchData(SiftData &siftData1, SiftData &siftData2, CudaImage &img) {
  ...
  for (int j = 0; j < numPts; j++) {
    ...
    int s = std::min(
        x, std::min(y, std::min(w - x - 2, std::min(h - y - 2, (int)(1.41 * sift1[j].scale)))));
    int p = y * w + x;
    p += (w + 1);
    for (int k = 0; k < s; k++)
